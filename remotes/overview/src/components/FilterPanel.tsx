...
import { Tag24Regular, MegaphoneLoud24Regular, MoreHorizontal24Regular } from '@fluentui/react-icons';
                        ...
                        <div className={styles.textContent}>
                            <div className={styles.titleContainer}>
                                ...
                                {data.metrics.length > 3 && (
                                    <Badge appearance="tint" size="small" className={styles.moreBadge} icon={<MoreHorizontal24Regular />}/>
                                )}
                            </div>
                            <div className={styles.metricsContainer}>
    ...
    const renderContent = () => {
        ...
        return itemsToRender.map((item) => (
            <PerformanceCard
                key={item.id}
                data={item}
                // ✨ ไอคอนจะเปลี่ยนไปตาม Tab ที่เลือก
                icon={isOfferTab ? <Tag24Regular /> : <MegaphoneLoud24Regular />}
                // ✨ ส่งฟังก์ชัน onClick ให้กับการ์ดเฉพาะเมื่ออยู่ใน Tab "Offers"
                onClick={isOfferTab ? () => onOfferClick(item.id) : undefined}
                // ✨ การ์ดจะถูกเลือก (มีเส้นขอบสีฟ้า) เฉพาะเมื่ออยู่ใน Tab "Offers"
